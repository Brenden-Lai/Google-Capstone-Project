...
## Creating a ride_length column in minutes

cyclistic_df$ride_length <- difftime(cyclistic_df$ended_at,cyclistic_df$started_at, units = "mins")

## Changing the ride_length strings into a numeric datatype
...
cyclistic_df_cleaned <- cyclistic_df %>% ## Removing any ride_length rows with values of <= 0  
...
cyclistic_df_cleaned %>% ## Counting the number of casuals and members
  group_by(member_casual) %>% 
  count(member_casual)

cyclistic_df_cleaned %>% ## Checking the total amount of bikes and the different types
...
cyclistic_df_cleaned %>% ## Checking the amount of riders each day of the week
  count(day_of_week) 

cyclistic_df_cleaned %>%  ## Seeing which days were the most popular to casuals vs members
  group_by(member_casual) %>% 
  count(day_of_week)
...
cyclistic_df_cleaned %>% ## Looking at which month casuals vs members spent the most time on 
  group_by(month, member_casual) %>% 
  summarise_at(vars(ride_length), list(time = mean)) %>%
  print(n = 24)

cyclistic_df_cleaned %>% ## Seeing the average ride length by month casuals + members
  group_by(month) %>% 
  summarise_at(vars(ride_length), list(time = mean)) 

aggregate(cyclistic_df_cleaned$ride_length ~ cyclistic_df_cleaned$member_casual + ## Average ride length for casuals vs members by day of week
            cyclistic_df_cleaned$day_of_week, FUN = mean) 

cyclistic_df_cleaned %>% ## Seeing the average ride_length of casuals vs members overall
  group_by(member_casual) %>%  
  summarise_at(vars(ride_length), 
               list(time = mean))

cyclistic_df_cleaned %>% ## Seeing the average ride length by day of week casuals + members
...
cyclistic_df_cleaned %>% ## Looks at the number of rides by day and the average ride length for casuals vs members
